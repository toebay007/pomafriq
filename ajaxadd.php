...
<?php  
    $conn =new  Mysqli('localhost','root','','poma');
    session_start();
   ...
   if($action == 'add'){
        ...
        if(!empty($p_id)){
            $query = "SELECT * FROM products WHERE products_id = '" . $p_id . "'";
            $rs = $conn->query($query);
            $product_data = $rs->fetch_assoc();

            $productItem = array("p_id" => $product_data['products_id'], "name" => $product_data['products_name'], "price" => $product_data['products_price'] , "image" => $product_data['products_img'], "quantity" => $quantity);

            if(isset($_SESSION['product_cart']) && !empty($_SESSION['product_cart'])){
                if(!array_key_exists($product_data['products_id'], $_SESSION['product_cart'])){

                    $_SESSION['product_cart'][$product_data['products_id']] = $productItem;
                } else{
                    $_SESSION['product_cart'][$product_data['products_id']]['quantity'] = $_SESSION['product_cart'][$product_data['products_id']]['quantity'] + $quantity;
                }
            } else{
                $_SESSION['product_cart'][$product_data['products_id']] = $productItem;
            }
